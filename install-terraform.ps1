...
This script attempts to (1) fix common Chocolatey permission problems,
then (2) install Terraform using `winget` if available, otherwise `choco`.

Run this script from an elevated PowerShell (Administrator). If not elevated
the script will re-launch itself elevated.

Usage (Admin PowerShell):
  PS> .\scripts\install-terraform.ps1

Note: The script only prepares and runs installers — you'll still need to
restart PowerShell after the install so `terraform` appears on PATH.
#>
...
if (-not (Test-IsElevated)) {
    ...
    $psi.Verb = "runas"
    try { [System.Diagnostics.Process]::Start($psi) | Out-Null } catch { Write-Error "Failed to elevate. Please re-run this script as Administrator."; exit 1 }
    exit 0
}
...
if (Test-Path $chocoData) {
    try {
        ...
        Write-Warning ("Failed to adjust permissions on {0}: {1}" -f $chocoData, $_)
    }
}
else {
    Write-Host "$chocoData does not exist; skipping permission fix." -ForegroundColor Yellow
...
if (Get-Command winget -ErrorAction SilentlyContinue) {
    Write-Host " found." -ForegroundColor Green
    Write-Host "Installing Terraform with winget..." -ForegroundColor Cyan
    # Prefer the official HashiCorp.Terraform package id
    winget install --id HashiCorp.Terraform -e --source winget
    if ($LASTEXITCODE -eq 0) { Write-Host "winget install completed." -ForegroundColor Green } else { Write-Warning "winget install returned exit code $LASTEXITCODE" }
}
elseif (Get-Command choco -ErrorAction SilentlyContinue) {
    Write-Host "winget not found; using Chocolatey (choco) to install Terraform..." -ForegroundColor Yellow
    choco install terraform -y
    if ($LASTEXITCODE -eq 0) { Write-Host "choco install completed." -ForegroundColor Green } else { Write-Warning "choco install returned exit code $LASTEXITCODE" }
}
else {
    Write-Host "Neither winget nor choco found. Offering manual-install guidance." -ForegroundColor Yellow
    Write-Host "Please download Terraform from https://releases.hashicorp.com/terraform/ and extract terraform.exe to a folder on your PATH (e.g., C:\tools\terraform)." -ForegroundColor Cyan
...
Write-Host "Installation attempted. You should close and re-open PowerShell to refresh PATH." -ForegroundColor Cyan
Write-Host "Verify by running: terraform -v" -ForegroundColor Cyan
